...
- **The browser check now gives up after 15 seconds** rather than leaving the Sign in button disabled forever. Cloudflare sometimes renders the check and then simply never answers — that is what it does to a browser it distrusts, and a few real ones (older in-app browsers, hardened privacy browsers) land in the same bucket. They were left with a button that could never be pressed and nothing explaining why.
