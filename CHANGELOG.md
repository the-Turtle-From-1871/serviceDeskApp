...
### Added
- Operational-readiness tracking on inventory items: each item now carries an `isAccountedFor` flag (defaults to accounted-for) and a `deployableStatus` state — one of `DEPLOYED`, `READY_TO_DEPLOY`, `IN_REPAIR`, or `RETIRED`.
- **Readiness analytics dashboard** at `/admin/analytics` (admin-only, linked from the admin hub). A global **Unit (UIC)** filter at the top re-scopes every widget on the page: audit readiness (accounted-for vs not), fleet KPIs (in-service vs ready, broken down by category), fleet status over time, DA Form 2062 velocity, and a unit-allocation leaderboard whose rows set the global filter. Time-series charts offer 30d / 90d / 6m / 1y ranges, and every chart can be exported to PNG or CSV or switched to a **table view** from its actions menu.
- **Device category** on items (`Laptops`, `Switches`, …). Importable via CSV — the importer accepts a `category`, `deviceCategory`, `type`, or `devicetype` column — and a change is logged to item history like `deviceUIC`. Backfill the existing catalogue by re-importing with a category column.
...
- **Readiness history.** Every change to `deployableStatus` / `isAccountedFor` is now recorded, which is what the status-over-time chart reads. History starts the day this shipped; the chart shows no invented past before that point.
