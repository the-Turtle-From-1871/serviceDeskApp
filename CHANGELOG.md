...
- The item card is presentation only — no query, no server action and no permission changed, and the buttons behind the swipe are the same admin-only actions that were already on the row. The Columns menu now shapes the desktop table only; the card is built from its own cells so that hiding the Serial column cannot leave a card with no heading and no way to open the item.
