...
### Added
- **Items now carry a storage location ("SLoc").** Any signed-in user can see it on an item's page. Admins can also set it — from the item page, from the admin edit page, and when creating a new item — with suggestions drawn from locations already in use on other items, the same way Make/Model/UIC already suggest. It can also be filled in by CSV import, using a `storageLocation` (or `SLoc`) column.
