...
### Changed
- **The app has a custom domain: `https://www.dcsim.us`.** The `*.vercel.app` hostname is DNS-sinkholed on the government network the nightly export runs from — it resolves to `127.0.0.1` there, so nothing on that network could reach the app at all, the automated import included. The import script, the hand-off document, the purge cron job and the deployment docs now name the new domain. The old `*.vercel.app` hostname still serves the app and is deliberately kept, so QR labels already printed keep working anywhere the block is not in force; labels scanned with the in-app scanner are unaffected either way, because it matches on the URL path and ignores the origin. **Set `APP_URL` to `https://www.dcsim.us` so newly printed labels carry it.**

### Fixed
- **The import script follows redirects, and refuses an HTML page even on a 200.** The apex `dcsim.us` permanently redirects to `www.dcsim.us`, and `curl` does not follow redirects unless told to, so the job failed with a bare `308`. It now sends `-L`. Because following redirects can land on a login page, the guard against an HTML body — previously reached only on a non-200 response — now runs on every response, including a 200. That is the case the document calls out as dangerous: without it the job reports success while importing nothing.
