...
### Fixed
- **Home units are no longer rewritten to `BE` — including on the devices already stuck that way.** Yesterday's fix stopped MDM's autogenerated names (`BE-7J5AKRNFLP6B`) from overwriting real device names, and said that fixed the home-unit damage with it. **It only fixed part of it.** A device's home unit is worked out from the start of its name, and the name is only kept where there is a real one to keep — so for a device whose stored name was *already* autogenerated, or that had no name, or that was brand new, the import still read `BE` out of the random name and wrote it to the home unit.

  Those are exactly the devices that were already broken, which made the damage self-sustaining: once a device had been renamed to `BE-…`, every later import read `BE` back out of that name, so it could never recover on its own — **and repairing the home unit by hand did not make it stick**, because the name it is derived from was still the autogenerated one.

  That is not hypothetical. The 18 devices that had been corrupted have since been put right, but **82 devices still carry an autogenerated name**, and measured against the live property book the next import would have set every one of their home units to `BE` — **overwriting a real unit on 67 of them and filling in `BE` on the 15 that were blank**, the just-repaired devices among them.

  The import now refuses to read a home unit out of an autogenerated name at all, on every path. There is no unit hidden in a random 12-character suffix, so nothing is lost by not looking: a device in that state keeps whatever home unit it already had, rather than having it overwritten or cleared, and it is not added to the "unrecognised unit" list either — there is nothing there for an administrator to teach.

  Renaming a device properly in Intune remains the real cure: the name decodes correctly again from the next import, and the *Needs rename in Intune* worklist is there to work through them.
...
- **A device's name is no longer replaced by a random one after it is re-imaged.** When a device is re-enrolled, MDM gives it an autogenerated name like `BE-7J5AKRNFLP6B`, and the import treated that as the truth — so a device called `NGHINBPMACN128` simply became `BE-7J5AKRNFLP6B` overnight. The import now keeps the name you gave it and records what MDM wanted instead. The item's page says so: *MDM calls this device BE-XXXX — rename it in Intune*. Rename it there and the note clears itself on the next import.
  ...
  **The same problem was quietly wrecking home units.** A device's home unit is worked out from the start of its name, so `BE-2AD6890X7IOL` matched a unit called `BE` and the device's home unit became `BE`. **18 devices are in that state now**, one of them previously `Safety Office`. Keeping the real name fixes this wherever there is a real name to keep — see the correction dated 2026-08-11, which covers the devices where there is not.
  ...
  **Notes:** adds a nullable `Item.mdmProposedName` column (migration `20260811000000_item_mdm_proposed_name`). Additive and safe to apply before deploy; no backfill — the flag fills in on the next import that meets a device MDM wants to rename.
