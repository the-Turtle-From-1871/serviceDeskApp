...
- **The public-access gate now refuses everyone if its signing key is missing, instead of admitting everyone.** With no `AUTH_SECRET` the unlock cookie's signature could be reproduced by anyone, so a misconfigured deploy would have let visitors past the PIN entirely. It now fails closed, and entering a PIN on such a deploy fails outright rather than appearing to work.
