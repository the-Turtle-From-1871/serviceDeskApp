...
- **A device named `NGHINB-LOAN-###` is now marked as a loaner automatically.** Naming is how loaner stock is already tracked, so it no longer has to be recorded twice. Rename a device *into* that pattern — in Intune, on the edit page, on the item card, or with the bulk rename — and it becomes a loaner on the spot. Rename it *out* of the pattern and the mark is removed. New devices arriving with that name are marked as they are created, and **the 51 devices already named that way have been marked**.
  ...
  **One thing to know:** because MDM owns device names, MDM now decides loaner status for any device whose name enters or leaves the pattern. A device renamed out of it in Intune loses the mark on the next import. That is the trade for not maintaining the same fact in two places. A re-imaged loaner is safe — the import already refuses MDM's autogenerated `BE-…` names, so the real name and the mark both survive.

  #### Notes
  - Migration `20260811223000_backfill_loaner_from_name` marks existing devices whose name matches. It only ever **sets** the flag, never clears one, so nothing marked by hand is overruled. **Apply it to Supabase before merging.**
