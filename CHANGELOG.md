...
- **Sign-in and password reset are now counted per account AND per network.** Five failed attempts on one account from one connection, under a ceiling of sixty attempts per connection overall. (The public PIN is different — one shared secret, so it is twenty attempts per network and correct entries count too.) The pairing is the point: counting only per connection would mean one person mistyping their password five times locks out everyone else at the desk, while counting only per account would let someone work through a list of addresses unchecked. The email is stored only as a one-way hash, never in readable form.
