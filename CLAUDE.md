...
* **Readiness AND accountability are both DERIVED — neither is a stored state.**
  * **Readiness** ("can it go out") is computed from live signals by `readinessState()` in `src/modules/items/readiness.ts`: `RETIRED` (lifecycle) > `IN_REPAIR` (a `PENDING` `ServiceQueueItem`) > `DEPLOYED` (an open unreturned hand receipt) > `DEPLOYED` (`lastLogonAt` newer than `markedReadyAt`) > `READY_TO_DEPLOY` (`markedReadyAt` set) > `DEPLOYED` (an MDM `lastLogonUserPrincipalName`) > `UNTRIAGED`. **That precedence is load-bearing** — checking the service flag before the receipt rule is what stops a device turned in for repair on an still-open receipt from reading "Deployed".
...
* **`Item.markedReadyAt` is the ONLY hand-set readiness signal, and it is a TIMESTAMP on purpose.** It means "this device is back on my shelf as of now" (`markItemsReady`). It self-expires: an MDM logon dated *after* the stamp outranks it, so the marking goes stale on its own instead of quietly persisting. Never replace it with a boolean, and never backfill it from receipt state.
...
* **Readiness is NOT sortable in the `/items` table** — it is derived from four signals across three tables, so there is no column to `ORDER BY`. It stays a visible/hideable column; fleet readiness composition is the analytics dashboard's job. (Contrast `auditState`, which *is* sortable because it rides the denormalized `Item.lastAuditedAt`.)
...
* **Readiness edits are ADMIN-only** and live in their own Server Action (`markItemsReadyAction`, the "Mark as on hand" button on `/items` and the item page), deliberately NOT folded into `updateItemDetailsAction`'s role-picked schema — that keeps the USER-editable field set exactly as narrow as it was (holder email + position only).
