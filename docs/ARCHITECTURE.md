...
### Item
`id, make, model, serialNumber (unique, citext), homeUnit?, deviceUIC? (unit issued-to code, indexed), deviceCategory? (free-text device class — "Laptops", "Switches"; indexed), storageLocation? (free-text shelf/bin location, no index), deviceName?, notes?, currentUserEmail?, currentPosition?, MDM telemetry (lastLogonUserPrincipalName?, lastLogonDate?, enrollmentDate?, compliance?, lastSyncDateTime? — imported, read-only; `lastSyncDateTime` is when MDM last checked in, a different question from `lastLogonDate`'s "when a person last signed in", and like it has a parsed twin — `lastSyncAt`, added 2026-08-11 when the dormant-device list moved onto it), mdmProposedName? (the autogenerated name an import REFUSED to apply — non-null means "rename this device in Intune", and it clears itself once MDM stops disagreeing; backs the `?needsRename=1` worklist on `/items`), status (ACTIVE|RETIRED), markedReadyAt? (the one hand-set readiness signal — see below), lastLogonAt? / lastSyncAt? (the MDM `lastLogonDate` / `lastSyncDateTime` text parsed to instants on import by one shared parser; the sync twin backs the dormant-device window, indexed for its range scan), lastAuditedAt? (denormalized audit recency — ALSO the accountability signal), createdById, timestamps`
