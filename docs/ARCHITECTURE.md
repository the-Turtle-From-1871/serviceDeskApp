...
### Item
`id, make, model, serialNumber (unique, citext), homeUnit?, deviceUIC? (unit issued-to code, indexed), deviceCategory? (free-text device class — "Laptops", "Switches"; indexed), storageLocation? (free-text shelf/bin location, no index), deviceName?, notes?, currentUserEmail?, currentPosition?, MDM telemetry (lastLogonUserPrincipalName?, lastLogonDate?, enrollmentDate?, compliance?, lastSyncDateTime? — imported, read-only; `lastSyncDateTime` is when MDM last checked in, a different question from `lastLogonDate`'s "when a person last signed in", and unlike it has no parsed twin because nothing computes over it), status (ACTIVE|RETIRED), markedReadyAt? (the one hand-set readiness signal — see below), lastLogonAt? (the MDM `lastLogonDate` text parsed to an instant on import), lastAuditedAt? (denormalized audit recency — ALSO the accountability signal), createdById, timestamps`
...
- **Delete, alongside Retire.** An admin can permanently delete an item (`deleteItem()`, `deleteItemAction`), not only retire it. There is no undo, and — unlike Retire — the row and its `ServiceQueueItem`/`ItemEdit`/`ItemAudit` history are gone (`ON DELETE CASCADE`). It is for rows that should never have existed (a mistyped serial duplicated by hand, a bad CSV row); Retire stays the reversible option for real equipment going out of service. See the `TransferLine`/`TransferItem` entry below for why a delete does not touch receipts, and [`SECURITY.md`](./SECURITY.md) §2 for the authorization boundary.
...
- **`TransferLine` / `TransferItem`** — a receipt's items, grouped by make/model (line) down to each serial (item). `TransferItem.returnedAt` is the per-item handback marker. **`TransferItem.itemId` is nullable, `ON DELETE SET NULL`** (not `Cascade`): a hand receipt is a signed document describing what was issued at the time, not a live view of the fleet, so deleting the `Item` it points at must detach the line rather than erase it. Every field a receipt actually renders — `serialNumber`/`make`/`model` (on the line) and the party signatures (on the `Transfer`) — is a snapshot written at creation, never joined back to `Item`, so a detached `TransferItem` (`itemId: null`) still displays and PDFs identically to one whose item still exists. No rendering path reads `TransferItem.itemId` itself; it exists only to let `listReceiptsForItem`/`getHoldingTransfer` join backward from a *live* item to its receipts, which correctly stops finding a deleted one.
...
- **`SignatureAsset`** — one signature image, stored once and keyed by the **SHA-256 of its bytes**. `ItemAudit` and `ReturnTransaction` reference it instead of each carrying a ~12 KB PNG data URL inline; a given admin's ink is byte-identical on every row they sign, so production held 31 audit rows carrying 1 distinct image and a full-fleet audit would have written ~16 MB to record one signature 1,204 times (measured: 16 MB inline vs 336 kB deduplicated over 1,204 rows, 49× smaller — TOAST cannot close that gap, because base64-of-PNG is already-compressed and incompressible). Content-addressing is what lets this be a *storage* change and not a weakening: the bytes behind a reference can never change, both FKs are `ON DELETE RESTRICT`, and nothing deletes from the table — so a signed history row keeps exactly the ink it was signed with even if the source `Signature` is edited or deleted, which is the same guarantee the inline snapshot gave. `Transfer.receiverSignature` deliberately stays inline: recipient ink is unique per receipt (nothing to deduplicate) and it is bound by the Ed25519 receipt seal.
