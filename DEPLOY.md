...
- `200` — the import ran, with a JSON summary. `added`, `updated` and
  `unchanged` are **counts** (numbers); `skipped` and `mismatches` are
  **arrays** of per-row detail, `[]` when there's nothing to report — don't
  call `.length` on the first three or iterate the last two as if they were
  counts. Example body:
  ...
  > **Changed 2026-08-11:** `detected` and `unresolved` were removed along with
  > home-unit auto-detection. A consumer that reads either will now get
  > `undefined` — check yours before upgrading.
...
- **A blank `homeUnit` cell imports as a blank home unit.** The import no longer
  guesses one from the device name (removed 2026-08-11 — it decoded `BE-…`
  autogenerated names into a unit called "BE" and corrupted 18 devices, while
  filling in exactly one legitimately). Put the unit in the CSV's `homeUnit`
  column, or set it in the app. Devices without one are listed on
  `/admin/units` under **Devices with no home unit**.
