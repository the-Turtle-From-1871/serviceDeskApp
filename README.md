...
- **Item registry** — make, model, serial number (**unique, case-insensitive**), device name, home unit, issuing unit (UIC), device category, storage location, current-holder email/position, notes; ACTIVE/RETIRED status. The list is **server-paginated, filterable (by UIC), and compound-sortable on every column** — including derived readiness, which is ordered in SQL rather than by a stored column. Every edit surface (new-item form, admin edit page, item detail card, identity card) suggests make/model/UIC/storage location from the fleet's own values and category/home unit from the managed lists as you type — a mobile-working combobox, not a `<datalist>` — while leaving every field free text.
