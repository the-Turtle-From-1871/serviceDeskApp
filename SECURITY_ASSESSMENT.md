...
6. **This is one nondeterministic static review plus a targeted dynamic pass.** It complements Semgrep, dependency scanning, and human code review; it does not replace them, and **it is not a comprehensive penetration test or a full dynamic application security test.** The static review found what a reader thought to look for; the dynamic pass confirmed only F2/F3/F5.

> **Recommended next step — a comprehensive dynamic test.** For breadth beyond the three findings verified by hand, run **Strix** — a dynamic application security testing / autonomous penetration-testing tool — against a **local instance, never production**. It crawls, fuzzes, and probes the running app end-to-end (public PIN-gated surface, the per-receipt capability token, the rate limiters, the CSV/import endpoints, and the PDF/QR routes), surfacing candidate findings that should then be triaged the same way this report's were — reachability, impact, adversarial "is this real" — rather than filed as raw tool output. **Guardrails:** local only (dynamic fuzzing/brute-force against `www.dcsim.us` is disruptive by design and unauthorised without an explicit decision); it cannot share the machine with active development (single dev server, truncating test DB); get authorization in writing before any run touching a shared or deployed environment. Tracked in `HANDOVER.md` as backlog **B23**.
