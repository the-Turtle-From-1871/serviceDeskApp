...
import { deriveNameHistory } from "@/modules/items/name-history";
...
export default async function PublicItemPage({ params }: { params: Promise<{ itemId: string }> }) {
  ...
  const [item, user, receipts, currentHolder, qr, service, lastEdit, nameEdits, audits, readiness] = await Promise.all([
    ...
    // Naming history, for the "previous names" disclosure on the details card.
    // BOUNDED and newest-first: deriveNameHistory only ever needs the most
    // recent real name and the superseded proposals, and an item edited for
    // years must not ship its whole audit trail into the RSC payload. Selects
    // two columns, not the row — `changes` is jsonb and the rest is unused.
        ...
        {loggedIn && (
          <ItemDetailsCard
            item={{
              ...
              // Derived server-side from the item's own edit history, because a
              // column overwritten in place remembers nothing. Two facts the
              // card offers behind a disclosure: what this device was called
              // before an autogenerated name replaced it, and which proposals
              // MDM has already moved on from.
              ...deriveNameHistory(nameEdits, item.mdmProposedName),
