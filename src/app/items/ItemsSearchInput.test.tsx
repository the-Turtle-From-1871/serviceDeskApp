...
describe("ItemsSearchInput", () => {
  it("debounces typing, then replaces the URL with q set and sort/dir preserved, no stale page", async () => {
    ...
    render(<ItemsSearchInput q="" sortKeys={[{ key: "serialNumber", dir: "asc" }]} uic={null} needsRename={false} loaner={false} showUnnamed={false} />);
  ...
  it("clearing the query replaces to /items with no q param", async () => {
    ...
    vi.useFakeTimers({ shouldAdvanceTime: true });
    const user = userEvent.setup({ delay: null });
    render(<ItemsSearchInput q="printer" sortKeys={[]} uic={null} needsRename={false} loaner={false} showUnnamed={false} />);
  ...
  it("mounting on a deep URL (page/sort/dir) does not navigate away, dropping page", async () => {
    ...
    vi.useFakeTimers({ shouldAdvanceTime: true });
    render(<ItemsSearchInput q="" sortKeys={[{ key: "make", dir: "asc" }]} uic={null} needsRename={false} loaner={false} showUnnamed={false} />);
  ...
  it("mounting with a non-empty q already in the URL does not navigate", async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    render(<ItemsSearchInput q="foo" sortKeys={[]} uic={null} needsRename={false} loaner={false} showUnnamed={false} />);
  ...
  it("uses the latest sort/dir, not a stale closure, if they change while the debounce is pending", async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    const user = userEvent.setup({ delay: null });
    const { rerender } = render(<ItemsSearchInput q="" sortKeys={[{ key: "serialNumber", dir: "asc" }]} uic={null} needsRename={false} loaner={false} showUnnamed={false} />);
    ...
    rerender(<ItemsSearchInput q="" sortKeys={[{ key: "make", dir: "desc" }]} uic={null} needsRename={false} loaner={false} showUnnamed={false} />);
  ...
  // that used to disappear: the unit filter and the SECOND key of a compound
  // sort.
  it("preserves the UIC filter and both sort keys while searching", async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true });
    const user = userEvent.setup({ delay: null });
    render(
      <ItemsSearchInput
        ...
        uic="W6BTAA" needsRename={false} loaner={false} showUnnamed={false}
      />,
    );
  ...
  it("pressing Enter does not trigger a full-page form submit", async () => {
    ...
    render(<ItemsSearchInput q="" sortKeys={[]} uic={null} needsRename={false} loaner={false} showUnnamed={false} />);
    const input = screen.getByRole("textbox", { name: /search/i });
