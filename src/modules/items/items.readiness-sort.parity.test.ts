...
type Seed = {
  ...
  /** True = pool stock, which is what the `loaner` worklist filter selects on. */
  isLoaner?: boolean;
...
const SEEDS: Seed[] = [
  ...
  { serial: `${PREFIX}06`, make: "Cisco", model: "C2", deviceName: "Node six", uic: "W1AAAA", category: "Laptop", lastAuditedAt: null, lastLogonUser: "a@b.mil", isLoaner: true, expected: "DEPLOYED" },
  ...
  // Carries an autogenerated name in `deviceName` itself — every other BE- value
  // in this fixture sits in `mdmProposedName`, which is a different filter. It
  // matches no search term, no uic and neither worklist, so it moves the "no
  // filter" size and nothing else.
  { serial: `${PREFIX}17`, make: "Getac", model: "K120", deviceName: "BE-EEEEEEEEEEEE", uic: null, category: null, lastAuditedAt: null, expected: "UNTRIAGED" },
];
...
const FILTERS: {
  name: string;
  opts: { search?: string; uic?: string; needsRename?: boolean; loaner?: boolean; hideUnnamed?: boolean };
  size: number;
}[] = [
  ...
  // Only 06 is marked isLoaner. The generic loop below already runs this
  // through every ORDER_CASE (a plain-column Prisma sort like serialNumber
  // AND the derived readiness sort that forces the raw path), so it pins the
  // property task 3 exists for: the two paths agree with the filter on, and
  // both exclude the unmarked rows.
  { name: "loaner filter", opts: { loaner: true }, size: 1 },
  ...
  // The default `/items` view. Drops the three null-named seeds (03, 07, 13) and
  // the autogenerated 17 — and it is the one filter here whose two
  // implementations are not a line-for-line mirror (Prisma has no regex, so both
  // sides carry a `BE-` prefix rule), which is exactly why it needs pinning on
  // both paths rather than only on the one it was written for.
  { name: "hide unnamed", opts: { hideUnnamed: true }, size: 13 },
  { name: "hide unnamed + uic", opts: { hideUnnamed: true, uic: "W1AAAA" }, size: 7 },
  // A SEARCH LIFTS THE HIDE, on both paths: same size as the bare "search term"
  // case above, so a lift applied to only one path could not pass this.
  { name: "hide unnamed + search lifts it", opts: { hideUnnamed: true, search: "delta" }, size: 7 },
];
