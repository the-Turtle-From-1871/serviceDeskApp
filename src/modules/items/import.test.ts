...
describe("planImport", () => {
  ...
  describe("autogenerated MDM device names", () => {
    ...
    /* The three cases withholding does NOT cover.
     *
     * Withholding only fires when there is a good stored name to fall back on,
     * so it leaves the autogenerated name in `effectiveDeviceName` whenever the
     * stored name is itself autogenerated, is absent, or the serial is new — and
     * those rows then decoded `BE` out of it. The two "accepts the autogenerated
     * name" tests above are exactly those cases but run against `UNITS`, which
     * has no `BE` row, so detection found nothing and the bug was invisible.
     * These re-run them against `UNITS_WITH_BE`, which is what production has.
     *
     * This is the shape that made the corruption self-sustaining: once a device
     * had been renamed to `BE-…`, its STORED name was autogenerated too, so
     * withholding stopped applying and every later import re-derived `BE`. */
    it("does not decode a home unit from the autogenerated name when the stored name is autogenerated too", () => {
      const { toUpdate } = planImport(
        [mk(1, { serialNumber: "S1", deviceName: AUTO, homeUnit: "" })],
        map("S1", existing({ deviceName: "BE-S64IL5PDENAZ", homeUnit: "Safety Office" })),
        UNITS_WITH_BE,
      );
      // The stored home unit is left ALONE — not rewritten to "BE", and not
      // cleared either.
      expect(toUpdate[0]?.data.homeUnit).toBeUndefined();
      expect(toUpdate[0]?.loggedChanges.map((c) => c.field)).not.toContain("homeUnit");
    });

    it("does not decode a home unit from the autogenerated name when there is no stored name", () => {
      const { toUpdate, unresolved } = planImport(
        [mk(1, { serialNumber: "S1", deviceName: AUTO, homeUnit: "" })],
        map("S1", existing({ deviceName: null, homeUnit: null })),
        UNITS_WITH_BE,
      );
      expect(toUpdate[0]?.data.homeUnit).toBeUndefined();
      // Nor reported as unresolved: that list asks an admin to teach an
      // abbreviation, and there is no unit hiding in a random suffix.
      expect(unresolved).toEqual([]);
    });

    it("does not decode a home unit from an autogenerated name on a NEW serial", () => {
      const { toCreate, unresolved } = planImport(
        [mk(1, { serialNumber: "NEW-1", make: "Dell", model: "5540", deviceName: AUTO, homeUnit: "" })],
        new Map(),
        UNITS_WITH_BE,
      );
    ...
    // The guard must not cost a genuine name its detection.
    it("still decodes a home unit from a real name when a BE unit exists", () => {
      const { toUpdate } = planImport(
        [mk(1, { serialNumber: "S1", deviceName: "HI-DCSIM-LT-001", homeUnit: "" })],
        map("S1", existing({ deviceName: "HI-DCSIM-LT-001", homeUnit: null })),
        UNITS_WITH_BE,
      );
      expect(toUpdate[0]?.data.homeUnit).toBe("DCSIM");
