...
describe("planImport", () => {
  ...
  describe("autogenerated MDM device names", () => {
    ...
    it("records the refusal in history, but never the rename that did not happen", () => {
      const { toUpdate } = planImport(
        [mk(1, { serialNumber: "S1", deviceName: AUTO })],
        map("S1", existing({ deviceName: "NGHINBPMACN128" })),
      );
      // The device was NOT renamed, so deviceName must not appear — a history
      // row for a change that did not happen would be a lie about the device.
      expect(toUpdate[0].loggedChanges.map((c) => c.field)).not.toContain("deviceName");
      // The refusal itself IS recorded: the column is overwritten in place, so
      // history is the only thing that remembers what MDM proposed before.
      expect(toUpdate[0].loggedChanges).toContainEqual({
        field: "mdmProposedName", from: null, to: AUTO,
      });
    });
    ...
    // A flagged device stays flagged across imports, and the flag tracks the
    // LATEST proposal. MDM re-enrols a device more than once, so the name it
    // wants can change while the device is still waiting to be renamed — a flag
    // naming a proposal MDM has since abandoned would send someone to Intune
    // looking for a device that is not called that any more.
    it("keeps a flagged device flagged when MDM proposes a DIFFERENT autogenerated name", () => {
      const SECOND = "BE-ZZ99YY88XX77";
      const { toUpdate } = planImport(
        [mk(1, { serialNumber: "S1", deviceName: SECOND })],
        map("S1", existing({ deviceName: "NGHINBPMACN128", mdmProposedName: AUTO })),
      );
      // Still withheld: the stored name is still the real one.
      expect(toUpdate[0]?.data.deviceName).toBeUndefined();
      // ...and the flag now names what MDM wants TODAY, not the stale proposal.
      expect(toUpdate[0]?.data.mdmProposedName).toBe(SECOND);
    });
