...
export type ItemsPage = {
  ...
  loaner: boolean;
  /** Whether the unnamed/autogenerated hide was ACTUALLY applied — the caller's
   *  `hideUnnamed` AND no search term. Echoed as the effective outcome rather
   *  than the request so no surface has to re-derive "a search lifts it": the
   *  summary chip and `hidden` below both read this. */
  hideUnnamed: boolean;
  /** How many rows the hide removed, or 0 when it was not applied. A total that
   *  silently omitted 158 devices would be a confident wrong answer about the
   *  property book — the count line renders this beside it. Costs one extra
   *  bounded COUNT, issued alongside the first, and only while the hide is on. */
  hidden: number;
};
...
function itemFilterSql(
  ...
  return Prisma.sql`
    ...
    AND (${hideUnnamed}::boolean IS NOT TRUE
      OR (i."deviceName" IS NOT NULL
        AND i."deviceName" <> ''
        AND i."deviceName" NOT LIKE 'BE-%'))`;
}
...
async function derivedOrderedItemIds(opts: {
  ...
  const rows = await prisma.$queryRaw<{ id: string }[]>(Prisma.sql`
    ...
    FROM "Item" i
    ${READINESS_JOINS}
    WHERE ${itemFilterSql(opts.search, opts.uic, opts.needsRename, opts.loaner, opts.hideUnnamed)}
...
export async function listItems(opts: {
  ...
  uic?: string | null;
  /** Show only devices whose name the importer refused to overwrite — i.e. MDM
   *  wants an autogenerated name for them and somebody should rename them in
  ...
  loaner?: boolean;
  /** Hide devices whose name identifies nothing — blank, or the `BE-…` name an
   *  MDM re-enrolment invents. Unlike the two worklist filters above this is a
   *  VIEW DEFAULT rather than a worklist (`/items` asks for it on every load and
   *  offers a checkbox to lift it), so it defaults to OFF here: a service that
   *  dropped rows unless a caller opted out is a footgun for the next caller.
   *
   *  A SEARCH LIFTS IT — the one product rule, and it lives here rather than in
   *  the page because the row list, the total and the hidden count all have to
   *  agree about which filter ran. Someone typing a serial is asking about one
   *  specific device and must not be told the property book has no such row. */
  hideUnnamed?: boolean;
} = {}): Promise<ItemsPage> {
  ...
  const hideUnnamed = opts.hideUnnamed === true && !search;
  ...
  // filter or items.readiness-sort.parity.test.ts fails — and it fails for a
  ...
  const combine = (fs: Prisma.ItemWhereInput[]): Prisma.ItemWhereInput | undefined =>
    fs.length === 0 ? undefined : fs.length === 1 ? fs[0] : { AND: fs };

  // Everything EXCEPT the unnamed hide. The hidden count below is the difference
  // between counting this and counting `where`, so the two must differ by
  // exactly that one clause.
  const baseWhere = combine(filters);
  // The Prisma twin of the clause in itemFilterSql, and the one filter here
  // whose two implementations are not a line-for-line mirror — Prisma has no
  // regex and no length operator (checked against the generated client for
  // 7.9.1), so the rule is a `BE-` PREFIX on both sides rather than the
  // importer's anchored `^BE-[A-Z0-9]{12}$`. On the live fleet the two select
  // the same 91 devices; a false positive here only hides a row from a default
  // view that a search or the menu's checkbox brings straight back, where in
  // `isAutogeneratedDeviceName` it would refuse a genuine rename for good.
  // Blank means NULL or "" exactly, not trimmed — `deviceName: { not: "" }` is
  // as close as Prisma gets, and a whitespace-only name shows on both paths
  // rather than on one.
  const where = hideUnnamed
    ? combine([
        ...filters,
        {
          AND: [
  ...
  // pager can never disagree about which filter was applied. The second count is
  // the same query without the unnamed hide — issued together, never serially,
  // and skipped entirely when the hide is off.
  const [total, unhiddenTotal] = await Promise.all([
    prisma.item.count({ where }),
    hideUnnamed ? prisma.item.count({ where: baseWhere }) : Promise.resolve(0),
  ]);
  const hidden = hideUnnamed ? unhiddenTotal - total : 0;
...
const UPDATABLE_ITEM_COLUMNS = new Set<string>([
  ...
  "compliance",
  "lastSyncDateTime",
  // The parsed twin of the column above, written by planImport whenever the raw
  // text changes. Omitting it here would throw on nearly every import, since MDM
  // syncs most devices most nights.
  "lastSyncAt",
  // Written silently by planImport when it withholds an autogenerated rename,
  // and cleared the same way. Omitting it here would throw on every import that
...
async function loadExistingBySerial(serials: string[]): Promise<Map<string, ExistingItem>> {
  ...
  const rows = await prisma.item.findMany({
    ...
    select: {
      ...
      // mdmProposedName is read so planImport can CLEAR a stale rename flag,
      // not just set one.
      enrollmentDate: true, compliance: true, lastSyncDateTime: true, mdmProposedName: true,
    },
  });
...
export async function commitImport(
  ...
  const { added, updated, skipped: finalSkipped } = await prisma.$transaction(async (tx) => {
    ...
    for (const { columns, rows: sigRows } of bySignature.values()) {
      ...
      // citext serialNumber column, and lastLogonAt/lastSyncAt cast to
      // timestamptz (see castFor) because they carry a Date, not a string.
