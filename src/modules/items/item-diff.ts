...
export type ItemLoggedFields = {
  ...
  // The autogenerated name the importer refused to apply, or null. LOGGED
  // (passed through `loggedAfter`), unlike the telemetry fields above it: the
  // column is overwritten in place, so ItemEdit is the only record of what MDM
  // proposed before, and the item page offers that history behind a disclosure.
  // Only transitions are written — raised, superseded, cleared — so a steady
  // state costs nothing. No form or schema exposes it for writing: `z.object()`
  // strips it from any submitted payload.
