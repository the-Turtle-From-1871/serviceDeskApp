...
export type NewItemImport = {
  ...
  lastSyncAt?: Date;
  /** Derived from the device NAME — see loaner-name.ts. A device created
   *  already following the `NGHINB-LOAN-###` convention is a loaner. */
  isLoaner?: boolean;
};
...
export type ItemUpdate = {
  ...
  itemId: string;
  serialNumber: string;
  // `string | Date | boolean` because the derived `lastLogonAt`/`lastSyncAt`
  // instants and the derived `isLoaner` flag travel alongside the raw text they
  // are computed from.
  data: Record<string, string | Date | boolean | null>;
...
export function planImport(
  ...
  for (const c of candidates) {
    ...
    if (match) {
      ...
      // THE LOANER FLAG FOLLOWS THE NAME, and only when the name actually
      // changed — `data.deviceName` is present only if diffItemFields emitted
      // it, so a withheld autogenerated rename (above) correctly moves nothing.
      //
      // That interaction is worth stating: a re-enrolled loaner comes back from
      // MDM as `BE-XXXXXXXXXXXX`, which does NOT match the convention. The
      // withhold guard keeps the stored `NGHINB-LOAN-###` name, so no rename is
      // written and the device stays a loaner. Without that guard this rule
      // would strip the flag off every re-enrolled loaner overnight.
      if (data.deviceName !== undefined) {
        const loaner = loanerUpdateForRename(match.deviceName, data.deviceName as string | null);
        if (loaner.isLoaner !== undefined) data.isLoaner = loaner.isLoaner;
      }
    ...
    const item: NewItemImport = {
      ...
      isLoaner: isLoanerName(d.deviceName),
    };
