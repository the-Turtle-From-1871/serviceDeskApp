...
/**
 * The name to decode a home unit FROM, or null when there is nothing decodable.
 *
 * An autogenerated name encodes NO unit. Its leading segment merely collides
 * with a real production `Unit` row named `BE`, so `detectHomeUnit` "succeeds"
 * and rewrites the device's home unit to `BE` — the corruption that hit 18
 * devices, one of them from `Safety Office`.
 *
 * Withholding the rename fixes the case where there is a good stored name to
 * fall back to, but it is NOT the whole fix, because withholding is deliberately
 * narrow: it does not happen when the stored name is ITSELF autogenerated, when
 * there is no stored name, or on the CREATE path. In all three the autogenerated
 * name is the one we keep, so it reaches detection — and those are precisely the
 * devices already corrupted, which is why they could never recover on their own.
 *
 * So the guard belongs HERE, at the decode, not at the rename: never decode a
 * unit out of a name that cannot contain one. Both call sites go through this.
 */
export function decodableDeviceName(name: string | null | undefined): string | null {
  if (!name) return null;
  return isAutogeneratedDeviceName(name) ? null : name;
}
...
export function planImport(
  ...
  for (const c of candidates) {
    ...
    if (match) {
      ...
      // An autogenerated effective name decodes to nothing — see
      // decodableDeviceName. Withholding covers only the case with a good stored
      // name to fall back on; this covers the rest.
      const decodable = decodableDeviceName(effectiveDeviceName);
      if (d.homeUnit !== undefined) {
        ...
        const full = detectHomeUnit(decodable, unitsByAbbrev);
        if (full) {
          ...
          unresolved.push({ row: c.row, deviceName: decodable, segments: splitSegments(decodable) });
        }
      }
      // An autogenerated name falls through to "leave it alone": homeUnit is
      // absent from loggedAfter, so the stored value is untouched rather than
      // overwritten with `BE` or cleared. It is deliberately NOT reported as
      // `unresolved` either — that list asks an admin to teach an abbreviation,
      // and there is no unit hiding in a random 12-character suffix.
    ...
    // Same guard as the UPDATE branch: a brand-new serial arriving under an
    // autogenerated name would otherwise be CREATED with homeUnit `BE`. There is
    // no withholding here — a new row has no stored name to prefer — so this is
    // the only thing standing between that name and detection.
    const creatableName = decodableDeviceName(item.deviceName);
    if (!item.homeUnit && creatableName) {
      const full = detectHomeUnit(creatableName, unitsByAbbrev);
      if (full) {
        item.homeUnit = full;
        detected++;
      } else {
        unresolved.push({ row: c.row, deviceName: creatableName, segments: splitSegments(creatableName) });
      }
    }
    toCreate.push(item);
  }
