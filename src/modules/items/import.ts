...
export function planImport(
  ...
  for (const c of candidates) {
    ...
    if (match) {
      ...
      } else if (effectiveDeviceName && !isAutogeneratedDeviceName(effectiveDeviceName)) {
        // An autogenerated name is never decoded — see the note on the CREATE
        // branch below. It is also not reported as `unresolved`, deliberately.
    ...
    // An AUTOGENERATED name is neither decoded nor offered for decoding, and
    // the second half matters as much as the first.
    //
    // detectHomeUnit scans every "-"/"_" segment, so `BE-2AD6890X7IOL` matches
    // a Unit named "BE" and writes it as the device's home unit — which is how
    // 18 production devices ended up in that state. The withhold guard above
    // does not cover this path: a brand-new serial has no stored name to
    // protect, so nothing is withheld.
    //
    // And an undecodable name lands in `unresolved`, where
    // /admin/items/import asks an admin to pick a segment and NAME it — which
    // is precisely how the "BE" unit came to exist in the first place. Listing
    // autogenerated names there invites someone to recreate the bug by hand.
    // The name carries no unit information, so there is nothing to resolve.
    if (!item.homeUnit && item.deviceName && !isAutogeneratedDeviceName(item.deviceName)) {
